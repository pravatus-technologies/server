...
class UserPreferences implements IUserPreferences {
	...
	public function getValues(string $userId, string $app, string $prefix = '', bool $filtered = false): array {
		...
		$this->loadPreferencesAll($userId);
		// array_merge() will remove numeric keys (here preference keys), so addition arrays instead
		$values = $this->formatAppValues($userId, $app, ($this->fastCache[$userId][$app] ?? []) + ($this->lazyCache[$userId][$app] ?? []));
		$values = array_filter(
			$values,
		...
		foreach ($values as $key => $value) {
			$result[$key] = $this->isTyped(ValueType::SENSITIVE, $this->valueTypes[$userId][$app][$key] ?? 0) ? IConfig::SENSITIVE_VALUE : $value;
		}
	...
	public function getAllValues(string $userId, bool $filtered = false): array {
		...
		$this->loadPreferencesAll($userId);
		// array_merge() will remove numeric keys (here preference keys), so addition arrays instead
		$cached = ($this->fastCache[$userId] ?? []) + ($this->lazyCache[$userId] ?? []);
		$result = [];
		foreach(array_keys($cached) as $app) {
			$values = $this->formatAppValues($userId, $app, $cached[$app]);
			if ($filtered) {
				foreach ($values as $key => $value) {
					$values[$key] = $this->isTyped(ValueType::SENSITIVE, $this->valueTypes[$userId][$app][$key] ?? 0) ? IConfig::SENSITIVE_VALUE : $value;
				}
			}
	 ...
	 * @param string $app id of the app
	 * @param string $key preference key
	 * @param string|array $value preference value, if ARRAY will within the list
	 * @param bool $caseInsensitive non-case-sensitive search, only works if $value is a string
	 *
	 * @return list<string>
	 * @since 31.0.0
	 */
	public function searchUsersByValueString(string $app, string $key, string|array $value, bool $caseInsensitive = false): array {
		$this->assertParams('', $app, $key, allowEmptyUser: true);
	...
	public function searchUsersByValueBool(string $app, string $key, bool $value): array {
		...
		return $this->searchUsersByValueString($app, $key, $values);
	}
	...
	private function getTypedValue(
		...
		$sensitive = $this->isTyped(ValueType::SENSITIVE, $knownType);
	 ...
	 */
	private function formatAppValues(string $userId, string $app, array $values, ?bool $lazy = null): array {
		foreach($values as $key => $value) {
			try {
				$type = $this->getValueType($userId, $app, $key, $lazy);
			} catch (UnknownKeyException $e) {
				continue;
			}
