...
async function fetchVersions(fileInfo) {
  ...
  try {
    ...
    .filter(({
      mime
    }) => mime !== '').map(version => formatVersion(version, fileInfo));
  } catch (exception) {
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,vue__WEBPACK_IMPORTED_MODULE_1__.defineComponent)({
  ...
  computed: {
    visibleSections() {
      ...
      const usedTokens = visibleItems.map(({
        key
      }) => key).filter(key => key !== undefined);
      const unusedTokens = Object.values(rowIdToKeyMap).filter(key => !usedTokens.includes(key));
      visibleItems.filter(({
        key
      }) => key === undefined).forEach(item => item.key = unusedTokens.pop() ?? Math.random().toString(36).substr(2));
      ...
      this._rowIdToKeyMap = visibleItems.reduce((finalMapping, {
        id,
        key
      }) => ({
        ...finalMapping,
...
/******/ })()
;
//# sourceMappingURL=files_versions-files_versions.js.map?v=d861ecaf3b56a599bc03